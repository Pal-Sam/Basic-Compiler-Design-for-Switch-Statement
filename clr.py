...
def pending_shifts(I):
    ...
    symbols = []
    # Will contain the symbols in order of evaluation
...
def CLR_construction(num_states):
    ...
    for Ii in C.values():
        ...
        # If the start production is in Ii
        if ('P->S.', '$') in Ii:
            ACTION.at[i, '$'] = "Accept"
...
def parse_string(string, ACTION, GOTO):
    ...
    while True:
        ...
        a = input_stack[ip]  # Current input symbol
        action = ACTION.at[S, a]  # New row to be added to the table

        # New row to be added to the table
        new_row = ["".join(stack), "".join(input_stack[ip:]), action]
        ...
        if 'S' in action:
            ...
            S1 = int(stack[-1])
            stack.append(A)
            stack.append(str(GOTO.at[S1, A]))  # Replace the number with the production for clarity
...
if __name__ == "__main__":
    ...
    string = None
    try:
        string = "".join(get_tokens("input.txt"))
    except:
        pass
